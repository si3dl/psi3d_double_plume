...
!-------------------------------------------------------------------------
   ...
   INTEGER :: maxcount = 4.E6,nn,is,ie,js,je,noH,isH,ieH,la,laaux,contBC,istat ! cintia
   INTEGER :: iter, itemp, i, j, niter1, p, Bstart, Bend, ide_thread, depth, mincol,liter,k,aux_la

   REAL :: t_exmom2, t_matmom2, t_matcon2, tsbar, tebar
...
!      print *,"entro en fdfrog",omp_get_thread_num ( )
    ...
    3 IF((nnodes > 0) .AND. (MOD(n,MAX(ipt,  1)) == 0)) CALL outt(n,thrs)
      ...
      IF((ipxml  < 0) .AND. (n .GE. itspf) .AND. (MOD(n,MAX(apxml,1)) == 0)) CALL outp(n)  ! 2nd condition added by ABH 2012, Jan 27 to write to file 
                                                                                           !(ptrack_hydro.bnr) after the initial time set in si3d_inp.txt
