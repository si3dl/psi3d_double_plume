...
!-----------------------------------------------------------------------
   ...
   !CALL InitializeScalarFieldsTWO !JCT
...
!-------------------------------------------------------------------------
   ...
   CASE DEFAULT
     ...
     END DO
     PRINT*, "Primera fila si3d_init: ",Scalardepthile(1,:)
...
!-------------------------------------------------------------------------
   ...
   INTEGER :: i, j, k, l, ios, imm1, jmm1, kmm1, ncols, ncols1, nc, &
              nsets, ia, ib, nn
   INTEGER :: npf, ils, ile, jls, jle, nci
   REAL    :: Vamp, rhoamp, Ts, Tb,   &  ! Used to initialize IW-problem
              NBV, meandepth, length, &
              rhohere, x, z, rhos, rhob
   CHARACTER(LEN=18)  :: initfmt
      ...
      DO liter = lhiW(omp_get_thread_num ( )+1), lhf(omp_get_thread_num ( )+1)
        ...
        ex(kmx+1:km1,l) = 0.0; 
         ...
         DO k = k1x,kmx
            ...
            !Avoid flow separation in deVriend's channel with high resolution dx = 0.02125
            

            ! Ribarroja downstream boundary (july) Cintia
            !IF ((j<=40)) THEN
            !   hdx  = 2.5*hdx
            !   hdx  = 10000*hdx
            !    hdx  = 50000*hdx
            ...
            ! Caso canal rotado 45º. 
            !IF ((i==59                 .AND. j==66                 )   .OR. & !CINTIA (SUTTER SLOUGH)
            !    (i==60                 .AND. j==65                 )   .OR. & !CINTIA (SUTTER GEO)
            !    (i==61                 .AND. j==64                 )   .OR. &
            !     (i==62                 .AND. j==63                 )    .OR. &
            !     (i==63                 .AND. j==62                 )    .OR. &
            !     (i==64                 .AND. j==61                 )    .OR. &
            !     (i==65                 .AND. j==60                 ))  THEN  !CINTIA (SUTTER DCC)   
            !  !corx = 0.0
            !  !advx = 0.0
            !  !hdx = 2.*hdx
            !  hdx = 1.E-2
            ...
            ! .......Gravity along x-direction (open channel flows) channel slope of 1/500. CINTIA
            ! gxh = g * (0.002) * hup(k,l)

            !.....Final explicit term.....
            ex(k,l) = uhpp(k,l) - twodt1*(advx*iadv-corx-hdx)
            !ex(k,l) = uhpp(k,l) - twodt1*(advx*iadv-corx-hdx-gxh) ! if channel slope Cintia
...
!-----------------------------------------------------------------------
   ...
   CASE (1)
      ...
      DO liter = lhiWCE(omp_get_thread_num ( )+1), lhfCE(omp_get_thread_num ( )+1)
            ...
            ! Increasing the bottom drag at the Ebro River during summer:
            !IF (i<=217)                 THEN  !CINTIA (EBRO)   
            !cd = 0.02
            !ENDIF
            
            ! Increasing the bottom drag Chu & Babarutsi
            !IF (j<=123)                 THEN  !CINTIA (EBRO)   
            !cd = 0.003
            !ENDIF
            
            ! Increasing the bottom drag Chu & Babarutsi (low resolution)
            !IF (j<=31)                 THEN  !CINTIA (EBRO)   
            !cdb = 0.00335
            !ELSE
            !cdb = cd
            !ENDIF
              ...
              DO innH = 1, iopssH(omp_get_thread_num ( )+1)
                ...
                IF (i == ipss(inn)     .AND. &
                  ...
                  DO k = k1x,kmx
                    ...
                    !print*,"x-Source Strength: ",Usource
   ...
   CASE (2)
      ...
      DO liter = lhiCN(omp_get_thread_num ( )+1), lhfCN(omp_get_thread_num ( )+1)
            ...
            ! .... Point sources and sinks .................................... 
            ! ACC commented Uhvalue and Vhvalue since cells in the linear diffuser and connected N-S, 2026
            IF ( iopssH(omp_get_thread_num ( )+1) > 0) THEN
...
!     i = l2i(l); j = l2j(l);
   ...
   !PRINT*, "no he roto todavía:" !cintia
   !print *,"qqmod:",sum(qq(:))
...
!-----------------------------------------------------------------------
   ...
   CASE (1) ! Compute wp from uh, uhpp, vh and vhpp
    !PRINT*, " Me he metido en continuity:" ! cintia
     DO liter = lhi(omp_get_thread_num ( )+1), lhf(omp_get_thread_num ( )+1)
...
!     i = l2i(l); j = l2j(l);
     ...
     DO k = k1s, kms;
        ...
        ! hd = 0 ! Changed to neglect diffusion of scalars in the horizontal Cintia
      ...
      CASE (2:)
         ...
         IF ( iopssH(omp_get_thread_num ( )+1) > 0 ) THEN
           ...
           IF (nt == ntr) THEN
              ...
              !  PRINT*, "modifiying DS with k4sdo, DS before, ", ds(kms)
              !ENDIF

              ! Sum k4sod for all point-source devices located at this (i,j)
              !tot_k4sod = 0.0
              !DO innH = 1, iopssH(omp_get_thread_num ( )+1)
              !  inn = ioph2iop(innH,omp_get_thread_num ( )+1)
              !  IF ( j /= jpss(inn) .OR. i /= ipss(inn) ) CYCLE
              !  !tot_k4sod = tot_k4sod + k4sod(inn)
              !  tot_k4sod = k4sod(inn)
              !ENDDO
              ...
              !IF (i == 193 .AND. j == 74) THEN
              !  PRINT*, "modifiying DS with k4sdo, DS after , ", ds(kms)
              !ENDIF

           END IF
         ENDIF
         !PRINT*, "otra vez:" ! cintia
