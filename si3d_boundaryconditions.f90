...
!------------------------------------------------------------------------
   ...
   DO nn = 1, nopen
      ...
      PRINT *,"nptsOpenBC:", nptsOpenBC  ! Debug
      ...
      END DO
       PRINT *, "primera fila: ",varsOpenBC(nn,:,1)! cintia
...
!------------------------------------------------------------------------
   ...
   CASE (1)
     ...
     PRINT*, "nº puntos surfbc.txt file: ", npsurfbc
     ...
     END DO
     PRINT*, "Primera línea surfbc.txt ",surfbc1(:,1)
...
!  ENDIF
  ...
  CASE (11) ! Interpolation
    ...
    DO l = 1, lm;
      ...
      ! ... Wind speed over the water column, ACC sets a guard
      ws  = SQRT (uair(l)**2.+vair(l)**2.);
      IF (ws > 10.0) THEN
         ws = 10.0
      END IF
...
END DO
  ...
  CASE (10) ! Interpolation
    ...
    DO liter = lhi(omp_get_thread_num ( )+1), lhf(omp_get_thread_num ( )+1)
     ...
     ! IF ((j<=25)) THEN !CINTIA WIND RIBARROJA JULIO
     ! ws  = 0.0
     ! ENDIF
