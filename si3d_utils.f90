...
!------------------------------------------------------------------------
   ...
   !PRINT*, "nº columns sources/sinks: ", iopss !cintia
  ...
  !PRINT*, "nº devices: ", npssdev !cintia
     IF (ios  /= 0) CALL input_error ( ios, 10 )
...
!------------------------------------------------------------------------
   ...
   ALLOCATE(  uout (km1) , vout (km1) , wout(km1),  &
            ...
            & uhout(km1) , scout(km1),  trout(km1,ntrmax), &
            & scoutm(km1,max_nprof) , &
			& troutm(km1,max_nprof,ntrmax) ,STAT=istat )
...
!-------------------------------------------------------------------------
   ...
   h4 = h4*0.1 + datadj
   PRINT *,'Second bathy cell (m)', h4(2,2) !Cintia
...
!-----------------------------------------------------------------------
   ...
   ! Protect against x values outside the range of available samples, ACC June 2026
   IF (x <= frstpt) THEN
      parab = fx(1)
      RETURN
   ELSE IF (x >= frstpt + REAL(SIZE(fx)-1, KIND=KIND(x)) * dx) THEN
      parab = fx(SIZE(fx))
      RETURN
   END IF
...
!-----------------------------------------------------------------------
   ...
   INTEGER :: kmzij, npf, i1, j1, l1, cont
   ...
   DO nn = 1, nnodes
      ...
      DO k  = k1, kmz(l)
		...
		PRINT *, 'Temperatura', k, scout(k)
		PRINT *, 'u (k, l)', u (k, l), k, l
       ...
       !       i1 = l2i(l1); j1 = l2j(l1); 
       !
       !       IF ( (i1 .GE. ilsb(npf)) .AND. &
       !       &    (i1 .LT. ileb(npf)) .AND. &
       !       &    (j1 .GE. jlsb(npf)) .AND. &
       !       &    (j1 .LT. jleb(npf)) ) THEN 
			 ! 
       !       cont = cont + 1              
       !       scoutm(k,npf) = scoutm(k,npf) + sal(k,l1)
			 ! IF (ntr>0) THEN
       !         DO it = 1, ntr 
       !           troutm(k,npf,it) = troutm(k,npf,it)+tracer(k,l,it)
       !         ENDDO
       !       ENDIF
		   !	
       !       ENDIF
       !     ENDDO
       !     scoutm(k,npf) = scoutm(k,npf)/cont
			 !IF (ntr>0) THEN
       !       DO it = 1, ntr 
       !         troutm(k,npf,it) = troutm(k,npf,it)/cont
       !       ENDDO
       !     ENDIF
		   !	
			 !PRINT *, 'scoutm','k','npf', scoutm(k,npf),k,npf
			 !PRINT *, 'troutm','k','npf','it', troutm(k,npf),k,npf,it
...
!     ENDIF
     ...
     DO m1 = 1, n_section_cells (k)
       ...
       DO m2 = k1, kmz(l)
         ...
         out_array(k_out,5) = up(m2,l) !uhp (m2,l) ! Changed 12/2010 SWA
         out_array(k_out,6) = vp(m2,l) !vhp (m2,l) ! Changed 12/2010 SWA
...
!   DEALLOCATE (out_array)
   ...
   ELSE
   !ELSEIF (n>=950400) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (Julio, dt=2s)
   !ELSEIF (n>=380160) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=5s)
   !ELSEIF (n>=1900800) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=1s)
   !ELSEIF (n>=475200) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=4s)
   !ELSEIF (n>=633600) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=3s)
   ! ELSEIF (n>=389100) THEN ! Cintia_Cambio para grabar condiciones a partir del día 9 (Feb, dt=2s)
    !  ELSEIF (n>=691500) THEN ! Cintia_Cambio para grabar condiciones a partir del día 16 (Feb, dt=2s)
   !ELSEIF (n>=604800) THEN ! Cintia_Cambio para grabar condiciones a partir del día 28 (Feb, dt=4s)
  ! ELSEIF (n>=380000) THEN !Cintia cambio Chu & Babarutsi
  ! ... Time stamp
   year_out = iyr
   mon_out  = imon
   day_out  = iday
...
!     ENDIF
     ...
     DO m1 = 1, n_section_cells(k)
       ...
       DO m2 = k1, kmz(l)
         k_out = k_out + 1
         out_array(k_out,1) = up(m2,l) !uhp (m2,l) ! Changed 12/2010 SWA
         out_array(k_out,2) = vp(m2,l) !vhp (m2,l) ! Changed 12/2010 SWA
         ...
         out_array(k_out,7) = q2p(m2,l) 		! cintia TKE
       END DO
     END DO
     ! ... Id # for plane file
     section_id = section_id0 + k
...
!-----------------------------------------------------------------------
   ...
   DO nn = 1, nxNBO
     ...
     ! ... Mass Balance Check
     WRITE (UNIT=iboid, FMT='(4E20.11)') thrs, uflow, dzi*dx*dy, dzi2*dx*dy 
...
!-----------------------------------------------------------------------
   ...
   ! ... Return if interior boundaries are not requested
   !IF (nopen <= 0) RETURN
   IF (nxNBO <= 0 .OR. ioNBO<= 0 .OR. ioNBTOGGLE <= 0) RETURN ! Cintia
...
!       ENDIF
       ...
       IF ( kp > km ) THEN
         DO c=1,cm1
           ...
           out_array(k_out,7) = 0.5*(Dv(kb,l)+Dv(kb+1,l))
...
!       DEALLOCATE (out_array)
  ...
  !ELSEIF (n>=950400) THEN ! Cintia_Cambio para grabar condiciones a partir del día 22 (Julio, dt=2s)
  !ELSEIF (n>=380160) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=5s)
  !ELSEIF (n>=1900800) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=1s)
  !ELSEIF (n>=475200) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=4s)
  ! ELSEIF (n>=633600) THEN ! Cintia Cambio para grabar condiciones a partir del día 22 (julio, dt=3s)
...
!-----------------------------------------------------------------------
   ...
   IF( n == 0 ) THEN
    ipoints = 0
    DO c = 1, cm1
      IF (.NOT. mask(c)) CYCLE        ! ACC delete output of dry cells
      l = l2c(c)
      DO k = k1, kmz(l) + 1
        IF (h(k,l) <= ZERO) CYCLE     ! ACC delete cells h=0
        ipoints = ipoints + 1
      ENDDO
    ENDDO
    ...
    DO k_t = 1, ntr
      ...
      DO c=1,cm1
        ...
        l = l2c(c)
        DO k = k1, kmz(l)
          IF (h(k,l) <= ZERO) CYCLE   ! ACC delete h=0 cells
          k_out = k_out + 1
          out_array(k_out, 1) = FLOAT(l2i(l))
          out_array(k_out, 2) = FLOAT(l2j(l))
          out_array(k_out, 3) = FLOAT(k)
          out_array(k_out, 4) = tracer(k, l, k_t)           ! mg/m3
          out_array(k_out, 5) = tracer(k, l, k_t) * h(k, l) ! mg/m2, compute mass by multiplying flux * dx * dy
        END DO
      END DO
      ! ... Id # for plane file
   ...
   ELSE
    ...
    DO k_t = 1, ntr
      ...
      DO c=1,cm1
        ...
        l = l2c(c)
        DO k = k1, kmz(l)
          IF (h(k,l) <= ZERO) CYCLE  ! ACC agregar filtro cuando h=0
          k_out = k_out + 1
          out_array(k_out, 1) = tracer(k, l, k_t) 
          out_array(k_out, 2) = tracer(k, l, k_t) * h(k, l)
        END DO
      END DO
      ! ... Id # for plane file
      tracer_id = tracer_id0 + k_t
...
!     ENDIF
     ...
     DO c=1,cm1
       ...
       DO k = k1, kmz(l)
         ...
         out_array(kout,8) = 0.5*(Dv(k,l)+Dv(k+1,l))
         !out_array(kout,8) = Dvm (k,l) !Andrea Ptrack.Cambio Dv por Dvm
         out_array(kout,9) = sal (k,l)!Andrea PT
         out_array(kout,10)= 0.5*(Av(k,l)+Av(k+1,l))!ptrack_Ribarroja
         out_array(kout,11)= tracer(k,l,1)!trazador
         out_array(kout,12)= s(l) ! ptrack_Ribarroja
         out_array(kout,13) = q2p(k,l)
       END DO;
     END DO
     ! ... Print time stamp followed by the records
     WRITE(ptrack_id) n,year_out,mon_out, day_out,hour_out,  &
     &              ((out_array(m1,m2),m2=1,13),m1=1,ipoints) ! cintia --> cambio 9 a 12
!     DEALLOCATE (out_array)
...
!     ENDIF
     ...
     DO c=1,cm1
       ...
       DO k = k1, kmz(l)
         ...
         out_array(kout,5) = 0.5*(Dv(k,l)+Dv(k+1,l))
         !out_array(kout,5) = Dvm(k,l) !Andrea Ptrack
         out_array(kout,6) = sal (k,l)
         out_array(kout,7) = 0.5*(Av(k,l)+Av(k+1,l))!ptrack_Ribarroja
         out_array(kout,8)= tracer(k,l,1)!trazador
         out_array(kout,9)= s(l) ! ptrack_Ribarroja
         out_array(kout,10) = q2p(k,l)
       END DO;
     END DO
     ! ... Print time stamp followed by the records
     WRITE(ptrack_id) n,year_out,mon_out, day_out,hour_out,  &
     &              ((out_array(m1,m2),m2=1,10),m1=1,ipoints) ! cintia cambio 6 a 9
!     DEALLOCATE (out_array)
...
!------------------------------------------------------------------------
   ...
   DO nn = 1, npssdev
	...
	! Read how water velocity at detrainment cell is calculated
        ...
        PRINT*,'flpss (nn)', nn, flpss(nn)  ! JCT
        PRINT*,'scpss (nn)', scpss(nn)  ! JCT! JCT
        PRINT*,'trpss(nn,itr)', nn, itr, trpss(nn,itr) ! JCT
